...
Then install `linkedin-campaign-operator` from the `sunny-linkedin-tools` marketplace. In Claude Code, the equivalent commands are:

```text
/plugin marketplace add sunny-chandel/linkedin-campaign-operator
/plugin install linkedin-campaign-operator@sunny-linkedin-tools
```

Invoke the parent skill with:

```text
/linkedin-campaign-operator:linkedin-campaign-orchestrator
```
...
```bash
claude --plugin-dir /absolute/path/to/linkedin-campaign-operator
```
