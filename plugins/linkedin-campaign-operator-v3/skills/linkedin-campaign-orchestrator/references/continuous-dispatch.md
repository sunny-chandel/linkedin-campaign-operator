...
Chrome loss, login failure, account-identity mismatch, unavailable capability, or a transient page failure enters automatic technical recovery. Retry safely up to two times, preserve external-outcome evidence, continue offline research, analytics, production, queue scoring from saved evidence, creative work, and logging, and run an automatic probe after cooldown. Resume the affected lane automatically after verification. Request owner input only when the dependency cannot be restored automatically and neither lane can advance.
