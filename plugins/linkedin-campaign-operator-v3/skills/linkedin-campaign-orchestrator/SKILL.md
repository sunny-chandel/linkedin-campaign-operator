...
Start with Chrome pre-flight. Read the connected LinkedIn account first so profile identity, niche, baselines, and available features can be discovered with minimal setup.
...
1. Call `list_connected_browsers` and confirm the intended Chrome device. If Chrome is unavailable or the device is wrong, block only the LinkedIn lane, ask the recognized owner only to connect or select the correct Chrome device, and continue valid offline work.
2. Open LinkedIn in the connected Chrome session and confirm it matches the owner name and profile URL stored in campaign state. On the first run, discover and store those values. Read the current profile name, handle, headline, image, niche, and visible positioning. If logged out or on the wrong account, block LinkedIn execution, ask the recognized owner only to correct the login, and preserve offline work that does not depend on new identity evidence.
