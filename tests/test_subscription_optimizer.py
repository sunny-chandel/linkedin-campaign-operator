...
class SubscriptionOptimizerTests(unittest.TestCase):
    ...
    def test_existing_campaign_migrates_without_overwriting_values(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("automatic-local-workspace", consent["persistent_settings"])
            self.assertIn("six-package-ready-inventory", consent["persistent_settings"])
            self.assertNotIn("rolling-160-action-target-200-cap", consent["persistent_settings"])
            ...
            self.assertEqual(migrated_state["engagement_scaling"]["base_daily_ceiling"], 200)
            operational = json.loads((state_dir / "operational-output.json").read_text(encoding="utf-8"))
            self.assertEqual(operational["actions"]["target"], 160)
            self.assertEqual(operational["publishing"]["target"], 6)
